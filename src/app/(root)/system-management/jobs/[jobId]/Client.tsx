...
export default function Client({ initialJob, initialProject }: Props) {
  ...
  const organizationId = initialProject?.clientOrganizationId ?? "";
  ...
  const getFieldValues = useCallback(
    ({ job }: { job: JobResponseDto | null }): FieldValues => {
      return {
        clientUser: {
          id: transformNullishStringIntoString.parse(
            job?.clientInfo.clientUserId
          ),
        ...
        additionalInformation: transformNullishStringIntoString.parse(
          job?.additionalInformationFromClient
        ),
        mountingType: transformNullishStringIntoString.parse(job?.mountingType),
        ...
        mailingAddress: {
          city: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.city
          ),
          coordinates: job?.mailingAddressForWetStamp?.coordinates ?? [],
          country: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.country
          ),
          fullAddress: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.fullAddress
          ),
          postalCode: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.postalCode
          ),
          stateOrRegion: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.state
          ),
          street1: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.street1
          ),
          street2: transformNullishStringIntoString.parse(
            job?.mailingAddressForWetStamp?.street2
          ),
        },
      };
    },
  ...
  const form = useForm<FieldValues>({
    ...
    defaultValues: getFieldValues({ job: initialJob }),
  });
  const {
