...
import { SheetState } from "./TasksDoneSection";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
const RELATIVE_PATH =
  "src/app/(root)/system-management/task-summary/TasksDoneDetailSheet.tsx";
...
export default function TasksDoneDetailSheet({
  state,
  fromDate,
  toDate,
  ...dialogProps
  ...
  const { data, isLoading } = useAssignedTasksSummaryDetailsQuery({
    params: {
      ...
      status: state.open ? state.status : undefined,
      startedAt: fromDate,
      endedAt: toDate,
      page: pageIndex + 1,
      limit: pageSize,
    ...
    <Sheet {...dialogProps} open={state.open}>
      <SheetContent className="sm:max-w-[1400px] w-full">
        <SheetHeader className="mb-6">
          <SheetTitle>
            Tasks {state.open ? state.status : "Completed"}
          </SheetTitle>
        </SheetHeader>
